            ...
            (result ? aid == bid : aid != bid); @*/
{
  ...
  bool addr_eq = ether_addr_eq(&id1->addr, &id2->addr);
  return addr_eq
...
struct nested_field_descr StaticKey_nests[] = {
  {offsetof(struct StaticKey, addr), offsetof(struct ether_addr , addr_bytes), sizeof(uint8_t ), 6, "addr_bytes"},
};
...
//@ ensures [f]StaticKeyp(obj, v) &*& result == _StaticKey_hash(v);
{
  ...
  unsigned hash = 0;
  unsigned addr_hash = ether_addr_hash(&id->addr);
