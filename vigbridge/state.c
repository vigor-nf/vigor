...
struct State* alloc_state(uint32_t capacity, uint32_t stat_capacity, uint32_t dev_count)
{
  ...
  ret->dyn_map = NULL;
  if (map_allocate(ether_addr_eq, ether_addr_hash, capacity, &(ret->dyn_map)) == 0) return NULL;
  ret->dyn_keys = NULL;
  if (vector_allocate(sizeof(struct ether_addr), capacity, ether_addr_allocate, &(ret->dyn_keys)) == 0) return NULL;
  ret->dyn_vals = NULL;
...
#ifdef KLEE_VERIFICATION
  map_set_layout(ret->dyn_map, ether_addr_descrs, sizeof(ether_addr_descrs)/sizeof(ether_addr_descrs[0]), ether_addr_nests, sizeof(ether_addr_nests)/sizeof(ether_addr_nests[0]), "ether_addr");
  vector_set_layout(ret->dyn_keys, ether_addr_descrs, sizeof(ether_addr_descrs)/sizeof(ether_addr_descrs[0]), ether_addr_nests, sizeof(ether_addr_nests)/sizeof(ether_addr_nests[0]), "ether_addr");
