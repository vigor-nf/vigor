...
#define DEFAULT_STATICKEY StaticKeyc(ether_addrc(0, 0, 0, 0, 0, 0), 0)

/*@
inductive StaticKeyi = StaticKeyc(ether_addri, uint16_t ); @*/

/*@
predicate StaticKeyp(struct StaticKey* ptr; StaticKeyi v) = 
  struct_StaticKey_padding(ptr) &*&
  ether_addrp(&ptr->addr, ?addr_f) &*&
  ptr->device |-> ?device_f &*&
...
fixpoint unsigned _StaticKey_hash(StaticKeyi x) {
  switch(x) { case StaticKeyc(addr_f, device_f):
    return crc32_hash(crc32_hash(0, _ether_addr_hash(addr_f)), device_f);
  }
} @*/
