...
static uint64_t nat_map_hash_fn_dpdk(void *key, void *key_mask,
                                     uint32_t key_size, uint64_t seed) {
  return (*map_hash_fn)(*((nat_flow_id *)key));
}

void nat_map_set_fns(nat_map_hash_fn hash_fn, nat_map_eq_fn eq_fn) {
  (void)eq_fn; // Unused

  map_hash_fn = hash_fn;
}

struct nat_map *nat_map_create(uint32_t capacity) {
  ...
  void *dpdk_table =
      rte_table_hash_ext_ops.f_create(&table_params, 0, sizeof(nat_flow *));
  if (dpdk_table == NULL) {
...
void nat_map_insert(struct nat_map *map, nat_flow_id key, nat_flow *value) {
  // The add function allows to both check if the value was already there, and
  // get a handle to the entry. We care about neither.
  int unused_key_found;
  void *unused_entry_ptr;

  int ret = rte_table_hash_ext_ops.f_add(map->value, &key, &value,
                                         &unused_key_found, &unused_entry_ptr);
  if (ret != 0) {
    rte_exit(ret, "Error in nat_map_insert\n");
  }
}

void nat_map_remove(struct nat_map *map, nat_flow_id key) {
  // Same remark as insert
  int unused_key_found;
  void *unused_entry_ptr;

  int ret = rte_table_hash_ext_ops.f_delete(map->value, &key, &unused_key_found,
                                            &unused_entry_ptr);
  if (ret != 0) {
...
bool nat_map_get(struct nat_map *map, nat_flow_id key, nat_flow **value) {
