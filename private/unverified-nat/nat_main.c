...
// ICMP support is not implemented, as this NAT only exists for benchmarking
// purposes; since the protocol type has to be checked anyway, an ICMP check
// would not significantly change performance.

static bool nat_flow_greater_timestamp(nat_flow *left, nat_flow *right) {
...
static std::priority_queue<struct nat_flow *, std::vector<struct nat_flow *>,
                           decltype(&nat_flow_greater_timestamp)>
    flows_by_time(nat_flow_greater_timestamp);

static time_t current_timestamp;

static struct nat_flow_id
nat_flow_id_from_headers(struct ipv4_hdr *header,
...
static void nat_flows_by_time_refresh(void) {
  // This only works because the default container of priority_queue is a
  // vector.
  std::make_heap(const_cast<nat_flow **>(&flows_by_time.top()),
...
int nf_process(struct rte_mbuf *mbuf, time_t now) {
  ...
  if (now > current_timestamp && !flows_by_time.empty()) {
    ...
    while ((current_timestamp - expired_flow->last_packet_timestamp) >
  ...
  if (in_port == config.wan_device) {
    ...
    struct nat_flow_id flow_id =
        nat_flow_id_from_headers(ipv4_header, tcpudp_header);
    NF_DEBUG("Flow: %" PRIu16 " -> %" PRIu16, flow_id.src_port,
             flow_id.dst_port);
