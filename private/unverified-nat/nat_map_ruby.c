...
static int ruby_comparer(st_data_t left, st_data_t right) {
  return map_eq_fn(*reinterpret_cast<nat_flow_id *>(&left),
                   *reinterpret_cast<nat_flow_id *>(&right));
}

static st_index_t ruby_hasher(st_data_t value) {
...
void nat_map_insert(struct nat_map *map, nat_flow_id key, nat_flow *value) {
  st_insert(map->value, *reinterpret_cast<st_data_t *>(&key),
            *reinterpret_cast<st_data_t *>(&value));
}
