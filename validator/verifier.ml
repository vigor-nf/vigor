...
let verify_ir ir verifast fname outf proj_root lino_fname =
  ...
  let _ = Sys.command (verifast ^ " -c -prover z3v4.5 -I " ^ proj_root ^ " -I ../libvig/stubs/dpdk" ^
                       " " ^ fname ^ " > " ^ outf)
  in
...
(* TODO: this is more thorough version, that checks separately for bugs in
   the call prefix preceding the tip call; and insonsistency
   (i.e. the incompatible assumptions).
   It helps to spot symbex overapproximations, incorrect symbex models,
   broken lemmas, contracts etc. However it slows down validation by a factor of 3, which
   has became unbearable with the huge amount of VigLB traces.*)
  (* let vf_succeded = Sys.command ("grep '0 errors found' " ^ outf ^
   *                                " > /dev/null")
   * in
   * let _ = Sys.command (verifast ^ " -c -prover z3v4.5 -I " ^ proj_root ^ " -I ../libvig/stubs/dpdk" ^
