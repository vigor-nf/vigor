...
/*@
  inductive dchain = dchain(list<pair<int, vigor_time_t> >, int,
                            vigor_time_t, vigor_time_t);
  ...
  fixpoint dchain empty_dchain_fp(int index_range, vigor_time_t low) {
  ...
  fixpoint vigor_time_t dchain_high_fp(dchain ch) {
    switch(ch) {case dchain(alist, size, l, h): return h;}
  }

  fixpoint bool same_index(int i, pair<int, vigor_time_t> b) {
    return i == fst(b);
  }
  ...
  fixpoint dchain dchain_allocate_fp(dchain ch, int index, vigor_time_t t) {
    switch(ch) { case dchain(alist, size, l, h):
  ...
  fixpoint list<pair<int, vigor_time_t> >
    remove_by_index_fp(list<pair<int, vigor_time_t> > lst, int i) {
      switch(lst) {
        case nil: return nil;
        case cons(h,t):
          return fst(h) == i ? t :
  ...
  fixpoint dchain dchain_rejuvenate_fp(dchain ch, int index, vigor_time_t time) {
    switch(ch) { case dchain(alist, size, l, h):
  ...
  fixpoint vigor_time_t alist_get_fp(list<pair<int, vigor_time_t> > al, int i) {
    switch(al) {
      case nil: return default_value<vigor_time_t>();
      case cons(h,t):
        return (fst(h) == i) ? snd(h) : alist_get_fp(t, i);
    }
  }

  fixpoint vigor_time_t dchain_get_time_fp(dchain ch, int index) {
    switch(ch) { case dchain(alist, size, l, h):
  ...
  fixpoint vigor_time_t dchain_get_oldest_time_fp(dchain ch) {
    switch(ch) { case dchain(alist, size, l, h):
  ...
  fixpoint bool is_cell_expired(vigor_time_t time, pair<int, vigor_time_t> cell) {
    return snd(cell) < time;
  }

  fixpoint list<int>
  get_expired_indexes_fp(vigor_time_t time, list<pair<int, vigor_time_t> > lst) {
    return map(fst, filter((is_cell_expired)(time), lst));
  }

  fixpoint list<int> dchain_get_expired_indexes_fp(dchain ch, vigor_time_t time) {
    switch(ch) { case dchain(alist, size, l, h):
      return get_expired_indexes_fp(time, alist);
    }
  }

  fixpoint dchain dchain_expire_old_indexes_fp(dchain ch, vigor_time_t time) {
    switch(ch) { case dchain(alist, size, l, h):
  ...
  fixpoint dchain expire_n_indexes(dchain ch, vigor_time_t time, int n) {
    switch(ch) { case dchain(alist, size, l, h):
      return dchain(fold_left(alist, remove_by_index_fp,
  ...
  fixpoint bool bnd_sorted_fp(list<vigor_time_t> times,
  ...
  lemma void index_range_of_empty(int ir, vigor_time_t l);
  requires 0 < ir;
  ...
  lemma void allocate_preserves_index_range(dchain ch, int index, vigor_time_t t);
  requires true;
