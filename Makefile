...
# Add the ether_addr file to autogen sources, always needed
NF_AUTOGEN_SRCS += $(SELF_DIR)/libvig/stubs/ether_addr.h

# Add the state file and autogenerated files to the NF sources
NF_FILES += state.c $(subst .h,.h.gen.c,$(NF_AUTOGEN_SRCS))

# NF base source
SRCS-y := $(SELF_DIR)/nf_main.c \
          $(SELF_DIR)/libvig/nf_time.c \
          $(SELF_DIR)/libvig/nf_util.c \
          $(SELF_DIR)/libvig/expirator.c \
          $(SELF_DIR)/libvig/containers/double-chain.c $(SELF_DIR)/libvig/containers/double-chain-impl.c \
          $(SELF_DIR)/libvig/containers/map.c $(SELF_DIR)/libvig/containers/map-impl.c \
...
# NF base stubs
VERIF_FILES += $(SELF_DIR)/libvig/stubs/containers/!(batcher)-stub.c $(SELF_DIR)/libvig/stubs/*-stub.c
# NF loop file
VERIF_FILES += loop.c
# Environment stubs
VERIF_FILES += $(SELF_DIR)/libvig/stubs/externals/*.c $(SELF_DIR)/libvig/stubs/core_stub.c $(SELF_DIR)/libvig/stubs/time_stub.c
...
VERIF_DPDK_INCLUDES += --include=libvig/stubs/builtin_stub.h --include=rte_config.h

# Files for DPDK
# Low-level stubs for specific functions
VERIF_DPDK_FILES := $(SELF_DIR)/libvig/stubs/dpdk_low_level_stub.c
...
symbex: clean autogen
	@$(COMPILE_COMMAND) $(VERIF_DEFS) -DVIGOR_STUB_DPDK $(VERIF_INCLUDES) -I $(SELF_DIR)/libvig/stubs/dpdk $(VERIF_FILES) $(VERIF_FLAGS)
	@$(LINK_COMMAND)
	@$(OPT_COMMAND)
	@$(VERIF_COMMAND) $(NF_VERIF_BASE_ARGS) $(NF_VERIF_ARGS)

symbex-fullstack: clean autogen
	@$(COMPILE_COMMAND) $(VERIF_DEFS) $(VERIF_DPDK_DEFS) -DVIGOR_STUB_HARDWARE $(VERIF_INCLUDES) $(VERIF_DPDK_INCLUDES) $(VERIF_FILES) $(VERIF_DPDK_FILES) $(SELF_DIR)/libvig/stubs/hardware_stub.c $(VERIF_FLAGS)
	@$(LINK_COMMAND)
	@$(OPT_COMMAND)
	@$(VERIF_COMMAND) $(NF_VERIF_BASE_ARGS) $(NF_VERIF_ARGS)

verify-dsos-nf.bc: clean autogen
	$(COMPILE_COMMAND) $(VERIF_DEFS) $(VERIF_DPDK_DEFS) $(NF_ARGUMENTS_MACRO) -DVIGOR_STUB_HARDWARE -DDSOS -D__linux__ -libc=none $(VERIF_INCLUDES) $(VERIF_DPDK_INCLUDES) $(VERIF_FILES) $(VERIF_DPDK_FILES) $(SELF_DIR)/libvig/stubs/hardware_stub.c $(SELF_DIR)/libvig/kernel/*.c $(VERIF_FLAGS) -mssse3 -msse2 -msse4.1
	ar x $(KLEE_UCLIBC)/libvig/libc.a
