...
#define DEFAULT_LOADBALANCEDBACKEND LoadBalancedBackendc(0, ether_addrc(0, 0, 0, 0, 0, 0), 0)

/*@
inductive LoadBalancedBackendi = LoadBalancedBackendc(uint16_t , ether_addri, uint32_t ); @*/
...
fixpoint unsigned _LoadBalancedBackend_hash(LoadBalancedBackendi x) {
  switch(x) { case LoadBalancedBackendc(nic_f, mac_f, ip_f):
    return crc32_hash(crc32_hash(crc32_hash(0, nic_f), _ether_addr_hash(mac_f)), ip_f);
  }
} @*/
