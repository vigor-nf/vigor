            ...
            (result ? aid == bid : aid != bid); @*/
{
  ...
  bool mac_eq = ether_addr_eq(&id1->mac, &id2->mac);
...
struct nested_field_descr LoadBalancedBackend_nests[] = {
  {offsetof(struct LoadBalancedBackend, mac), offsetof(struct ether_addr , addr_bytes), sizeof(uint8_t ), 6, "addr_bytes"},
};
...
//@ ensures [f]LoadBalancedBackendp(obj, v) &*& result == _LoadBalancedBackend_hash(v);
{
  ...
  unsigned mac_hash = ether_addr_hash(&id->mac);
