...
/*@
  ...
  predicate packetp(void* p, list<int8_t> unread, list<pair<int8_t*, int> > missing_chunks) =
    ...
    true == missing_chunks(missing_chunks, p, (int8_t*)p + borrowed_len(missing_chunks)) &*&
...
/*@
  lemma void borrowed_len_nonneg(list<pair<int8_t*, int> > missing_chunks, int8_t* start, int8_t* beginning)
