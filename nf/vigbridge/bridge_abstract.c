...
/*@
lemma void vector_get_values_index_of<t>(list<pair<t, real> > v, list<int> indices, t entry)
...
/*@
lemma void vector_get_values_known<t>(list<pair<t, real> > v, list<int> indices, int idx)
...
/*@
lemma void two_indexes_into_engaged_nondistinct(list<pair<ether_addri, real> > keys, int i1, int i2)
requires i1 != i2 &*&
         0 <= i1 &*& i1 < length(keys) &*&
         0 <= i2 &*& i2 < length(keys) &*&
    ...
    case cons(h,t):
      switch(h) { case pair(val, owned):
        if (i1 == 0) {
          ...
          real owned2 = snd(nth(i2, keys));
          filter_mem(pair(val, owned2), t, engaged_cell);
          mem_map(pair(val, owned2), filter(engaged_cell, t), fst);
        } else if (i2 == 0) {
          switch(nth(i1 - 1, t)) { case pair(aaa,bbb): }
          real owned1 = snd(nth(i1, keys));
          filter_mem(pair(val, owned1), t, engaged_cell);
          mem_map(pair(val, owned1), filter(engaged_cell, t), fst);
        } else {
...
/*@
lemma void synced_pairs_map_get(list<pair<ether_addri, real> > keys,
          ...
          if (key_to_erase == addr) {
            ...
            assert gen_dyn_entries(map_erase_all_fp
                   ...
                   gen_dyn_entries(map_erase_all_fp(t, vector_get_values_fp(keys, indices)),
                                   ...
                                   dchain_erase_indexes_fp(ch, cons(idx, indices)));
            if (index != idx) {
              assert snd(nth(index, keys)) != 1.0;
              assert fst(nth(index, keys)) == addr;
              assert fst(nth(idx, keys)) == addr;
              assert snd(nth(idx, keys)) != 1.0;
...
/*@
lemma void erase_address_preserves_mem(list<dyn_entry> l,
                                       ether_addri addr,
                                       ether_addri del_addr)
        ...
        mem(addr, map(get_dyn_addr, l));
{
  switch(l) {
    ...
    case cons(h,t):
      switch(h) { case dyn_entry(cur_addr,b,c):
        if (cur_addr != addr) {
          erase_address_preserves_mem(t, addr, del_addr);
        }
      }
  }
}
...
/*@
lemma void erase_address_remove_erased(list<dyn_entry> l, dyn_entry x, ether_addri addr)
requires get_dyn_addr(x) == addr;
ensures erase_address(l, addr) == erase_address(remove(x, l), addr);
{
  switch(l) {
    case nil:
    case cons(h,t):
      switch(h) { case dyn_entry(cur_addr,b,c):
        erase_address_remove_erased(t, x, addr);
      }
  }
}
@*/
    ...
    case cons(h,t):
      switch(h) { case dyn_entry(cur_addr, index, time):
        if (addr == cur_addr) {
          ...
          erase_address_remove_erased(table2, h, addr);
        } else {
          multiset_eq_map(get_dyn_addr, table1, table2);
          multiset_eq_distinct(map(get_dyn_addr, table1), map(get_dyn_addr, table2));
          map_remove(get_dyn_addr, h, table2);
  ...
  switch(dyn_map) {
    ...
    case cons(h,t):
      switch(h) { case pair(cur_addr, cur_index):
        ...
        nth_update_unrelevant(cur_index, index, pair(port, 1.0), vals);
                             ...
                             addr, port, time)) == true;
{
  ...
  nth_update(index, index, pair(port, 1.0), vals);
