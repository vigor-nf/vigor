...
//  from memory is dependant on the completion of the previous load. It avoids
...
// x must be of type uint8_t
// #define POLY8 (0x1d)   // From CRC-8-SAE J1850
// #define POLYSHIFT8(x) ( ((x) << 1) ^ ((static_cast<int8_t>((x)) >> 7) & POLY8) )
...
#define kPageSize  4096		// Must be a power of two
#define kPageSizeMask (kPageSize - 1)

// Make an array bigger than any expected cache size
#define kMaxArraySize (40 * 1024 * 1024)
...
#define kMinLgStride 4
#define kMaxLgStride 12
...
uint8_t* AllocPageAligned(int bytesize, uint8_t** rawptr) {
  ...
  *rawptr = (uint8_t*)(malloc(newsize));
  uintptr_t temp = (uintptr_t)(*rawptr);
  temp = (temp + kPageSizeMask) & ~kPageSizeMask;
  return  (uint8_t*)(temp);
}

// Zero a byte array
void ZeroAll(uint8_t* ptr, int bytesize) {
  memset(ptr, 0, bytesize);
}

// Fill byte array with non-zero pseudo-random bits
void PseudoAll(uint8_t* ptr, int bytesize) {
  uint32_t* wordptr = (uint32_t*)(ptr);
  int wordcount = bytesize >> 2;
  uint32_t x = POLYINIT32;
...
Pair* MakeLongList(uint8_t* ptr, int bytesize, int bytestride, bool makelinear) {
  ...
  for (int i = 1; i < element_count; ++i) {
    ...
    Pair* nextptr = (Pair*)(ptr + ((nextelement * bytestride) ^ extrabit));
...
// Read all the bytes
void TrashTheCaches(const uint8_t* ptr, int bytesize) {
  // Fill up array with pseudo-random nonzero values
  const uint64_t* uint64_tptr = (const uint64_t*)(ptr);
  ...
  if (gNeverZero == 0) {printf("sum = %ld\n", sum);}
}

int64_t NaiveTiming(uint8_t* ptr, int bytesize, int bytestride) {
  const Pair* pairptr = (Pair*)(ptr);
...
int64_t LinkedTiming(uint8_t* ptr, int bytesize, int bytestride, bool makelinear) {
  ...
  if (gNeverZero == 0) {printf("pairptr->data = %ld\n", pairptr->data);}

  return elapsed >> 8;			// cycles per load for 256 loads
}

int64_t LinearTiming(uint8_t* ptr, int bytesize, int bytestride) {
  bool makelinear = true;
  return LinkedTiming(ptr, bytesize, bytestride, makelinear);
}

int64_t ScrambledTiming(uint8_t* ptr, int bytesize, int bytestride) {
  bool makelinear = false;
  return LinkedTiming(ptr, bytesize, bytestride, makelinear);
}

int64_t ScrambledLoads(const Pair* pairptr, int count) {
  // Unroll four times to attempt to reduce loop overhead in timing
 uint64_t startcy = __rdtsc();
...
void  FindCacheSizes(uint8_t* ptr, int asdf, int linesize) {
  bool makelinear = false;
  const Pair* pairptr = MakeLongList(ptr, asdf, linesize, makelinear);
  ...
  for (int lgcount = 4; lgcount <= 19; ++lgcount) {
    ...
    // Try to force the data we will access out of the caches
    TrashTheCaches(ptr, asdf);

   // Repeat four times. First will take cache misses, others will not if small enough
    printf("lgcount[%d] load N cache lines, giving cy/ld. Repeat.  ", lgcount);
    for (int i = 0; i < 4; ++i) {
      int64_t cyclesperload = ScrambledLoads(pairptr, count);
      printf("%ld ", cyclesperload);
    }
    printf("\n");
  }
}

void  FindCacheAssociativity(uint8_t* ptr, int asdf, int linesize, int totalsize) {
  // You get to fill this in.
  printf("FindCacheAssociativity(%d, %d) not implemented yet.\n", linesize, totalsize);
}
...
void mystery2(void) {
  ...
  uint8_t* rawptr;
  uint8_t* ptr = AllocPageAligned(kMaxArraySize, &rawptr);
  ...
  for (int lgstride = kMinLgStride; lgstride <= kMaxLgStride; ++lgstride) {
    ...
    // Do naive timing
    int64_t naive_cyclesperload = NaiveTiming(ptr, kMaxArraySize, stride);

    // Do linear linked list timing
    int64_t linear_cyclesperload = LinearTiming(ptr, kMaxArraySize, stride);

    // Do scrambled linked list timing
    int64_t Scrambled_cyclesperload = ScrambledTiming(ptr, kMaxArraySize, stride);

    printf("stride[%d] naive %ld cy/ld, linear %ld cy/ld, scrambled %ld cy/ld\n",
            stride,
